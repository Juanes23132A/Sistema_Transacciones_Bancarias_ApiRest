...
export class transaccionesRepositorie {
    async Transacciones(transacciones: TransferenciasDepositos) {
        ...
        const querySql1 = `INSERT INTO transacciones (usuario_destino_id, cuenta_destino_id, monto, tipo_transaccion) VALUES  (?, ?, ?, ?)`;
        const querySql2 = `UPDATE cuentas_usuario SET saldo = saldo + ? WHERE cuenta_id = ? AND usuario_id = ?`;
        const values1 = [transacciones.usuario_destino_id, transacciones.cuenta_destino_id, transacciones.monto, TipoTransaccion.DEPOSITO]
        const values2 = [transacciones.monto, transacciones.cuenta_destino_id, transacciones.usuario_destino_id]
